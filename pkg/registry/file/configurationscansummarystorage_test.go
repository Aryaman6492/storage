...
func TestConfigurationScanSummaryStorage_GetList(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "no existing objects return empty list",
			args: args{
				key: "/spdx.softwarecomposition.seclogic.io/configurationscansummaries",
			},
		},
		{
			name: "existing object is returned",
...
func TestGetNamespaceFromKey(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "namespace1",
			key:      "/spdx.softwarecomposition.seclogic.io/ConfigurationScanSummary/namespace1",
			expected: "namespace1",
		},
		{
			name:     "no namespace",
			key:      "/spdx.softwarecomposition.seclogic.io/ConfigurationScanSummary/",
			expected: "",
		},
	}
